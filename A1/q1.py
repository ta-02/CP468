...
def gen_eightpuzzle_state() -> Optional[List[List[int]]]:
    puzzle_state = [[-1] * 3 for _ in range(3)]
    domain = DOMAIN[:]
    shuffle(domain)
    ...
    for row in puzzle_state:
        for num in row:
            puzzle_flat.append(num)

    return puzzle_state if is_solvable(puzzle_flat) else None


def gen_100_eight_puzzle_states() -> List[List[List[int]]]:
    num_puzzles = 0
    puzzle_states = []

    while num_puzzles < 100:
        puzzle_state = gen_eightpuzzle_state()
        if puzzle_state:
...
def serialize_state(state: List[List[int]]) -> Tuple[Tuple[int, int, int], ...]:
    serialized = []
    for row in state:
...
def main() -> None:
    puzzles = gen_100_eight_puzzle_states()
    results = []
    ...
    with open("q1results.txt", "w+") as f:
        header = (
