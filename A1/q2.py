...
def gen_fifteen_puzzle_state() -> Optional[List[List[int]]]:
    puzzle_state = [[-1] * 4 for _ in range(4)]
    domain = DOMAIN[:]
    shuffle(domain)
...
def gen_100_fifteen_puzzle_states() -> List[List[List[int]]]:
    num_puzzles = 0
    puzzle_states = []

    while num_puzzles < 100:
        puzzle_state = gen_fifteen_puzzle_state()
        if puzzle_state:
...
def main() -> None:
    puzzles = gen_100_fifteen_puzzle_states()
    results = []
